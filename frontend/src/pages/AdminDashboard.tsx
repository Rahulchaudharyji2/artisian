...
export default function AdminDashboard() {
    ...
    useEffect(() => {
        const fetchArtisans = async () => {
            ...
            try {
                const response = await fetch('http://localhost:5000/api/admin/artisans', {
    ...
    return (
        <div className="flex-1 flex flex-col bg-stone-50 w-full">
            <header className="bg-white p-6 border-b border-stone-200 flex items-center justify-between shadow-sm z-10 w-full max-w-full">
                                ...
                                {artisans.length === 0 ? (
                                    ...
                                    artisans.map(artisan => (
                                        <tr key={artisan._id} className="border-b border-stone-100 hover:bg-stone-50">
                                            ...
                                            <td className="p-4 text-stone-600">{artisan.region}</td>
                                            <td className="p-4 text-stone-600">{artisan.craft}</td>
                                            <td className="p-4 text-stone-600">{artisan.likes}</td>

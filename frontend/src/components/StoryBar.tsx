...
export default function StoryBar() {
    ...
    useEffect(() => {
        const fetchStories = async () => {
            try {
                // Fetch latest artisans for the story bar
                const response = await fetch('http://localhost:5000/api/artisans?limit=12');
                const data = await response.json();

                if (data && Array.isArray(data)) {
                    const formattedStories = data.map((artisan: any) => ({
                        id: artisan._id,
                        name: artisan.name.split(' ')[0], // First name only for brevity
                        craft: artisan.craft,
    ...
    return (
        <div className="w-full bg-white md:bg-transparent overflow-x-auto border-b border-stone-200 md:border-none flex items-center gap-4 py-4 px-4 scrollbar-hide">
            <div className="flex flex-col items-center gap-1 cursor-pointer min-w-max">
                <div className="w-16 h-16 rounded-full bg-stone-100 flex items-center justify-center border-2 border-stone-200 relative">

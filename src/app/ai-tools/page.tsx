            ...
            <div className="text-center max-w-2xl mx-auto">
                ...
                {!image ? (
                    ...
                    <div className="space-y-6">
                        ...
                        <div ref={actionRef} className="fixed bottom-0 left-0 right-0 p-4 bg-white/80 backdrop-blur-xl border-t border-stone-100 flex justify-center z-50 md:relative md:bg-transparent md:border-0 md:p-0">
                            {!isEnhanced ? (
                                <button 
                                    onClick={process}
                                    disabled={isProcessing}
                                    className="w-full max-w-md bg-blue-600 text-white p-5 rounded-2xl font-black text-lg hover:bg-blue-700 shadow-xl shadow-blue-200 transition-all flex items-center justify-center gap-3 animate-pulse cursor-pointer"
                                >
                ...
                <div className="grid md:grid-cols-2 gap-8 items-start">
                    <div className="space-y-4">
                        ...
                        >
                            {image ? (
                                <>
                                    <img src={image} alt="Upload" className="w-full h-full object-contain bg-stone-50" />
                                    <button onClick={() => setImage(null)} className="absolute top-4 right-4 bg-black/50 text-white p-2 rounded-full hover:bg-black transition-colors">
                                        <XIcon size={18} />
                                    </button>
                                </>
                            ) : (
            ...
            {image && (
                <div className="sticky bottom-0 left-0 right-0 p-4 bg-white/60 backdrop-blur-xl border-t border-stone-100 flex justify-center z-50 -mx-4 md:-mx-8">
                    <button 
                        ...
                        className="w-full max-w-sm bg-amber-600 disabled:opacity-50 text-white p-5 rounded-2xl font-black text-lg flex items-center justify-center gap-3 hover:bg-amber-700 shadow-xl shadow-amber-50 transition-all active:scale-95 group cursor-pointer"
                    >
...
function PriceSenseTool() {
    ...
    const handleAnalyze = async () => {
        setIsLoading(true);
        try {
            const res = await fetch('/api/ai/price-sense', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                    ...
                    <div className="space-y-6">
                        ...
                        <div className="space-y-2">
                            ...
                            <input 
                                ...
                                className="w-full p-5 bg-white border-2 border-stone-50 rounded-2xl outline-none focus:border-emerald-500 transition-all font-bold text-base shadow-sm text-stone-800"
                            />
                        </div>
                    </div>
            ...
            {image && (
                <div className="sticky bottom-0 left-0 right-0 p-4 bg-white/60 backdrop-blur-xl border-t border-stone-100 flex justify-center z-50 -mx-4 md:-mx-8">
                    <button 
                        ...
                        className="w-full max-w-sm bg-emerald-600 disabled:opacity-50 text-white p-5 rounded-2xl font-black text-lg flex items-center justify-center gap-3 hover:bg-emerald-700 shadow-xl shadow-emerald-50 transition-all active:scale-95 group cursor-pointer"
                    >
...
function CraftBoostTool() {
    ...
    const handleBoost = async () => {
        setIsLoading(true);
        try {
            const res = await fetch('/api/ai/craft-boost', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ images: [image] })
            });
        ...
        <div className="space-y-6 pb-20">
                ...
                {!image ? (
                    ...
                    <div className="w-full space-y-6">
                         <div className="flex flex-col md:flex-row gap-6 items-center bg-stone-50 p-4 rounded-[32px] border border-stone-100">
                             <div className="w-32 h-32 rounded-[24px] overflow-hidden shadow-md border-4 border-white shrink-0 relative">
                                 <img src={image} alt="Upload preview" className="w-full h-full object-cover" />
                                 <button onClick={() => setImage(null)} className="absolute top-1 right-1 bg-black/50 text-white p-1 rounded-full"><XIcon size={14}/></button>
                             </div>
            ...
            <div className="fixed bottom-0 left-0 right-0 p-4 bg-white/60 backdrop-blur-2xl border-t border-stone-100 flex justify-center z-50 md:relative md:bg-transparent md:border-0 md:p-0 md:mt-4">
                <button 
                    disabled={!image || isLoading}
                    onClick={handleBoost}
                    className="w-full max-w-sm bg-purple-600 disabled:opacity-50 text-white p-4.5 py-4 rounded-2xl font-black text-base flex items-center justify-center gap-2.5 hover:bg-purple-700 shadow-xl shadow-purple-50 transition-all active:scale-95 group cursor-pointer"
                >
    ...
    return (
        <div className="bg-stone-50 p-5 rounded-2xl border border-stone-100 flex flex-col h-full min-h-[140px]">
            ...
            {content && (
                <button 
                    onClick={onCopy}
                    className="text-[9px] font-black text-stone-400 hover:text-stone-900 flex items-center gap-1 transition-colors self-end uppercase cursor-pointer"
                >
                    <Check size={10} /> Copy
                </button>
            )}
...
// Ensure unique X icon export internally if needed, but not exporting globally.
